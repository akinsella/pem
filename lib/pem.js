 ...
 *
 * @param {Object} [options] Optional options object OR {Number} [keyBitsize=2048] Size of the key, defaults to 2048bit
 * @param {Object} [options] Optional options object
 * @param {String} [options.keyPassword] Optional key password
 * @param {String} [options.encryptAlgorithm] Optional encryptAlgorithm: aes128|aes192|aes256|camellia128|camellia192|camellia256|des|des3|idea
 * @param {Function} callback Callback function with an error object and {key}
 */
function createPrivateKey(options, callback) {

    if(!callback) {
        if ((typeof options == 'string' || options instanceof String)) {
            options = { keyBitsize: options };
        }
    ...
    var keyBitsize = Number(options.keyBitsize) || 2048;
    ...
    if (options.encryptAlgorithm) {
        params.push("-" + options.encryptAlgorithm);
    }
...
function createCSR(options, callback){
    ...
    if(!options.clientKey){
        options.keyBitsize = options.keyBitsize || 2048
        createPrivateKey(options, function(error, keyData){
            if(error){
                return callback(error);
            }
 ...
 * @param {String} [options.certPassword] Optional cert password
...
function createCertificate(options, callback){
    ...
    //    params.push("-passout");
    //    params.push("pass:" + options.certPassword);
    //}
 ...
 */
function getPublicKey(certificate, options, callback){
